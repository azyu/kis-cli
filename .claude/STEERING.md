...
- 2026-03-07: GitHub Actions는 `bb-cli`와 같은 최소 패턴으로 추가한다. `ci.yml`은 `fmt/test/release build`만 수행하고, `release-build.yml`은 수동 `release_tag` 입력으로 `kis_<version>_linux_amd64.tar.gz`와 `checksums.txt`를 GitHub Release에 업로드한다. Homebrew tap/formula 자동화는 이 마일스톤에 포함하지 않는다.
