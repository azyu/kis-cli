...
- `market`: 국내 거래량 순위/휴장일, 해외 거래량 순위/시가총액 순위
...
- 시세 조회(`price`, `quote ask`, `quote ccnl`, `chart daily`, `chart time`, `info detail`, `market volume --exchange`, `market cap`)는 시세용 거래소 코드(`NAS`, `NYS`, `AMS`, `TSE`, `HKS`, `SHS`, `SZS`, `HSX`, `HNX`)를 사용합니다.
